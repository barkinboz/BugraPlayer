/*
    ONcelikle Sa Buğra
    İnternetten hazır bulduğum kodları birleştirdim
    Play biraz sıkıntılı ilk başta çozemedim açıkçası pek de sikimde değil
...
var FramePlayer = function(el) {
    ...
    this.elem = el;
    this.frameSource = 'http://storage.googleapis.com/alyo/assignments/images';
    this.rate = 10,
    this.paused = true,
    this.width = '700px',
    this.height = '500px';
...
//frame process falan yaz gelir saniyede on oluyor sanırım boyle ama bılmıyom
...
//buna hiç elleme kanka sadece o split_25in ğrılıdığı yerle falan oyna bunu pek bozma bu da hazır
// nereden aldığımı unuttuma da http isteği yapıyor aslında gerek yok ama o istek bize zaman kazandırıyor sıktır et dursun orda
...
function split_25(frames, image) {
    let w2 = image.width / 5;
    let h2 = image.height / 5;
    for (let i = 0; i < 5; ++i) {
        for (let j = 0; j < 5; ++j) {
...
// Bu tamamen dışarıdan alındı kardeşim hiç bir fikrim yok polyfill yazıp bak çok da bibok yemıyor zaten
